...
Pulllog の Laravel 12（PHP 8.2）アプリです。API 中心のバックエンドで、Vite/Tailwind で最小限の資産をビルドします。主な依存は `laravel/sanctum`, `doctrine/dbal`, `intervention/image`, `reliese/laravel`。
...
例:
1. まずログイン: `curl -X POST http://localhost:3030/api/v1/auth/login -H "x-api-key: <key>" -H "Content-Type: application/json" -d "{\"email\": \"<user.email>\", \"password\": \"<user.password>\"}"`
2. ログイン後 1. のレスポンスから CSRFトークン を付与してAPIリクエスト: `curl -H "x-api-key: <key>" -H "x-csrf-token: <token>" -H "Content-Type: application/json" http://localhost:3030/api/v1/apps`
3. CSRFトークンに不備があると 419 レスポンスが返る: `curl -H "x-api-key: <key>" -H "x-csrf-token:" -H "Content-Type: application/json" -I http://localhost:3030/api/v1/apps`
